...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['BPDSurv'],
                    default='BPDSurv', help='Type of model')
...
parser.add_argument('--bag_loss',        type=str, choices=['svm', 'ce', 'ce_surv', 'nll_surv',
                    'cox_surv'], default='nll_surv', help='slide-level clas sification loss function (default: nll_surv)')
...
if 'survival' in args.task:
    ...
    if combined_study in ['tcga_blca', 'tcga_brca', 'tcga_ucec', 'tcga_luad']:
...
exp_code = str(args.exp_code)

print("===="*30)
print("Experiment Name:", exp_code)
print("===="*30)

#  args.results_dir/args.which_splits/args.param_code/exp_code
...
def main(args):
    ...
    summary_all_folds = {}
    for i in folds:
        proctitle = "BPDSurv_" + args.task.split('_')[1] + "[" + str(i) + "/4]"
        start_t = timer()
        seed_torch(args.seed)

        args.results_pkl_path = os.path.join(args.results_dir, 'split_latest_val_{}_results.pkl'.format(i))
        ...
        source_train_dataset, source_val_dataset = dataset.return_splits(from_id=False,
                                                                          csv_path='{}/splits_{}.csv'.format(
                                                                            args.split_dir, i))
        args.omic_sizes = source_train_dataset.omic_sizes

        train_dataset = source_train_dataset
        val_dataset = source_val_dataset
        ...
        if 'omic' in args.mode or args.mode == 'cluster' or args.mode == 'graph' or args.mode == 'pyramid':
            ...
            print('Genomic Dimensions (Model Architecture):', args.omic_sizes)
        else:
            args.omic_input_dim = 0


        if args.task_type == 'survival':
            best_val_dict, max_c_index, epoch_max_c_index = train(datasets, i, args,proctitle) ### new

        # Write Results for Each Split to PKL
        save_pkl(args.results_pkl_path, best_val_dict)
        summary_all_folds[i] = (max_c_index, epoch_max_c_index)
        end_t = timer()
    ...
    for i, k in enumerate(summary_all_folds):
        c_index, c_index_index = summary_all_folds[k]

        line = "Fold {}, C-Index: {:.4f}, epoch: {}".format(k, c_index, c_index_index)
        print(line)
    ...
    line = "Avg C-Index of {} folds: {:.3f}, stdp: {:.3f}, stds: {:.3f}".format(
