...
def collate_survival_coattn(batch):
    """
    过滤 None batch
    """
...
def plot_four_expert_distributions(k_pos_data, k_neg_data, q_pos_data, q_neg_data, save_path):
    """
    绘制 2x2 的专家使用率对比图
    """
    experts = ['ReLU', 'GELU', 'ELU']
    colors = ['#ff9999', '#66b3ff', '#99ff99']  # 粉、蓝、绿
    ...
    # 定义数据和标题的映射
    data_map = [
        (k_pos_data, "WSI (pos)", axes[0, 0]),
        (k_neg_data, "WSI (neg)", axes[0, 1]),
        (q_pos_data, "Gene (pos)", axes[1, 0]),
        (q_neg_data, "Gene (neg)", axes[1, 1])
    ]

    for data, title, ax in data_map:
        # 1. 计算统计量
        # data shape: (N_samples, 3)
        if len(data) == 0:
            continue

        counts = np.sum(data, axis=0)  # [sum_relu, sum_gelu, sum_elu]
        total = np.sum(counts) + 1e-8
        percentages = counts / total

        # 2. 画饼图
        patches, texts, autotexts = ax.pie(percentages, labels=experts, autopct='%1.1f%%',
                                           startangle=140, colors=colors, explode=explode,
                                           shadow=True, textprops={'fontsize': 11})
        ax.set_title(title, fontsize=14, fontweight='bold')

    plt.suptitle("Activation Distribution", fontsize=20, y=0.98)
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    print(f"专家分布对比图已保存至: {save_path}")
...
def collate_MIL_survival_sig(batch):
    ...
    case_id = [item[10] for item in batch]  # 这是一个 list of strings
    return [img, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c,case_id]
...
def get_custom_exp_code(args):
    ...
    if(args.max_epochs!=20):
        param_code += '_epoch'+str(args.max_epochs)
