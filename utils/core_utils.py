...
def train(datasets: tuple, cur: int, args: Namespace,proctitle):
    """
    ...
    if args.log_data:
        ...
        print("TensorBoard writing to:", writer.logdir)
    else:
        writer = None
    ...
    proctitle_index = 0
    proctitle_max = args.max_epochs
    for epoch in range(args.start_epoch,args.max_epochs):
        proctitle2 = proctitle+f"[{proctitle_index}/{proctitle_max}]"
        setproctitle.setproctitle(proctitle2)
        proctitle_index = proctitle_index+1
        from trainer.coattn_trainer import train_loop_survival_coattn, validate_survival_coattn
        train_loop_survival_coattn(
            epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn, reg_fn, args.lambda_reg, args.gc,
            args
            )
        val_latest, c_index_val, stop = validate_survival_coattn(cur, epoch, model, val_loader, args.n_classes,
                                                                 early_stopping, monitor_cindex, writer, loss_fn,
                                                                 reg_fn, args.lambda_reg, args.results_dir, args)
    ...
    return best_val_dict, max_c_index, epoch_max_c_index