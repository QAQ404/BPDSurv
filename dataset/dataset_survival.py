...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    ...
    def filter_missing_pt_cases(self):
        """
        在 dataset 初始化时：
        删除所有没有 pt 特征的 case
        """

        print("\n[Filtering missing PT cases...]")
        ...
        for case_id, slide_ids in self.patient_dict.items():
            ...
            for slide_id in slide_ids:
                ...
                base_id = normalize_slide_id(slide_id)
                ...
                if os.path.exists(pt_path):
                    ...
                    missing_slide_count += 1

            # ⭐ 至少要有一个 slide
            if len(valid_slides) > 0:

                valid_patient_dict[case_id] = valid_slides
                valid_case_ids.append(case_id)

            else:

                missing_case_count += 1
                print(f"[Removed Case] {case_id}")

        # 更新 patient_dict
        self.patient_dict = valid_patient_dict

        # 更新 slide_data
        self.slide_data = self.slide_data[
            self.slide_data['case_id'].isin(valid_case_ids)
        ].reset_index(drop=True)

        print(f"\nRemaining cases: {len(valid_case_ids)}")
        print(f"Removed cases: {missing_case_count}")
        print(f"Missing slides: {missing_slide_count}")
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        # base_id = normalize_slide_id(slide_id)
                        # wsi_path = os.path.join(data_dir, f"{base_id}.pt")
                        wsi_path = os.path.join(data_dir, '{}.pt'.format(slide_id.rstrip('.svs')))

                        if not os.path.exists(wsi_path):
                            print(f"[Warning] Missing PT: {wsi_path}")
                            continue

                        wsi_bag = torch.load(wsi_path,map_location=torch.device('cpu'),weights_only=False)
                        # print(wsi_path, wsi_bag.shape)

                        # base_id = normalize_slide_id(slide_id)
                        # wsi_path = os.path.join(data_dir, f"{base_id}.h5")
                        # with h5py.File(wsi_path, "r") as f:
                        #     wsi_bag = f["features"][:]
                        # if wsi_bag.ndim == 3 and wsi_bag.shape[0] == 1:
                        #     wsi_bag = wsi_bag[0]

                        if isinstance(wsi_bag, np.ndarray):
                            wsi_bag = torch.from_numpy(wsi_bag)
                        wsi_bag = wsi_bag.float()

                        if wsi_bag.numel() == 0:
                            print(f"[Warning] Empty feature: {wsi_path}")
                            continue
                    ...
                    if len(path_features) == 0:
                        print(f"[Error] No valid slides for case: {case_id}")

                        # dummy_dim = 1536
                        # path_features = torch.zeros((1, dummy_dim))
                        return None

                    else:
                        feat_dim = path_features[0].shape[1]

                        path_features = [
                            f for f in path_features
                            if f.shape[1] == feat_dim
                        ]

                        if len(path_features) == 0:
                            print(f"[Error] Feature dim mismatch for case: {case_id}")
                            path_features = torch.zeros((1, feat_dim))

                        elif len(path_features) == 1:
                            # 只有一个 slide，不用 cat
                    ...
                    return (path_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c,case_id)
                else:
